...
1. 项目必须保持无常驻服务器、无数据库、每日一次完整采集、三次 pending 重试的 GitHub Actions 的平民化部署方式。
...
6. FULL 与 RETRY 按配置时区每天最多发送一封汇总，包含未发送的新公告/更新和仍有效的到期提醒。关注 IP 仍有 pending 时暂缓发送，最后一轮 RETRY（--final-retry）允许原帖兜底；pending 清空可提前发送。每日回执阻止当天所有后续邮件，新增事项跨日保留。关闭每日必发且没有待通知事项时不发送。北京时间 02:00 完整采集，04:00、06:00、08:00 重试。候选通知与未来日期提醒分开，后者必须有可靠日期。
...
- 邮件按收件人独立发送：首次失败最多再重试 3 次，耗尽后后续每轮只尝试一次，成功恢复重试。`state/email-delivery.json` 只保存日期和 bitmap，不保存邮箱或地址摘要；位序对应去重后的 NOTIFY_EMAIL 顺序，收件人变更须同步迁移位序。
