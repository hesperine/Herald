...
日常第一轮接收同 IP 已存企划的精简状态，选择零或一个候选。程序同时按 IP 与明确合作方确定默认归属；有匹配企划时调用第二轮，支持标量 updates、new_activities、已有活动的 new_actions / new_venues。第二轮无法确定关系或选择 create_new 时，在同一合作方企划下保留新的 Activity；未验证的事实更新仍留 pending，不按名称相似度硬合并。候选原帖通知在 AI 前保存，失败也可进入邮件和网页每日提醒，但无可靠日期时不生成未来提醒。
...
- `notify_immediately=false` 只用于兼容历史提取分组，不再禁止通知。首次历史候选合并进入一封汇总；后续仅补充未发送的新信息。
- 提示词修改不会自动重跑已处理帖子。重建需明确清理运行状态，不要删除 local.env 或本地 Provider 密钥文件。
- 七天通知快照与长期防重回执分开保存；候选解析完成后，历史提醒可以关联到当前详情。
