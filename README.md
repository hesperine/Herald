...
米游社 / 森空岛内置官号 + 用户补充微博 UID
  ...
  读取作者时间线并补全每条帖子详情（中国区）
              ↓
  来源去重 → 规则初筛 → 保存候选通知 → 用户自己的 AI
                              │                │
                     原帖兜底等待每日汇总   失败保留 pending 重试
              ↓
 Campaign（联动企划）
...
每天按配置时区最多发送一封汇总，包含尚未发送的新公告/更新及仍有效的到期提醒。完整采集和前两轮重试后，关注 IP 仍有 pending 就暂缓发送；pending 清空后即可发送。08:00 最后一轮即使仍有 pending，也发送含原帖兜底的汇总。关闭每日必发时，没有待通知事项就不发；开启时允许空日报，同样受等待条件和每日一封限制。当天发过后，后续新增事项保留到次日。

多官号发布同一材料时会先去重；同一 `IP × 品牌` 的不同期次、全国商品、上海快闪和线上预约归入一个 Campaign。Activity 关联无法确认时保留为不同活动，未知日期、地点继续留空。未知合作方的材料不会统一合并到一个“未知品牌”企划。

## 最终输出

数据分为三层：爬虫的 `SourceObservation` 保存公开证据；`state` 中的
`Campaign → Activity → Action` 保存事实，`candidates` 保存公开候选与通知身份，日期队列保存提醒目标；`page` 是展示投影。
`page/data/active.json` 是有效企划目录，`page/data/today.json` 是今日通知卡片；每日提醒页读取 `page/data/reminders/` 的七天快照。
今日通知与邮件按 Campaign ID 和 Activity ID 聚合：同一子活动的多个变更、预约和
开售提醒合成一张卡片，具体 Action 仍保留独立队列任务与发送回执。企划级预告没有
Activity 时单独展示。候选原帖通知独立展示，AI 尚未成功时标记“待解析”，不凭空生成活动卡片、企划详情或未来日期任务。发送邮件不会使页面通知消失；过期详情会移除，七天内的提醒历史仍可回看原帖。

通知生成时读取当前活动事实。改期通过重新编排删除旧日期任务、写入新日期任务；
同一日期重复编排不会重复增加任务。同一来源编辑可以复用企划身份；子活动名称、
类型一致且城市不冲突、匹配唯一时，可在补充地点后沿用原 Activity/Action ID。
同一品牌的不同期次可共用企划，但不会仅凭品牌强行合并子活动。已发送的候选在提取成功后不会重复发同一公告；新增日期、参与入口等尚未发送的事实才触发补充通知。
模型提取范围包含游戏内跨 IP 联动；普通版本更新不属于目标范围。
已有提取缓存不会因提示词修改自动失效。FULL/RETRY 会幂等修复旧 state：补回历史未通知候选与缺失日期任务、合并同合作方企划并保留旧链接与回执，无需清空状态。历史已被删除的回执无法恢复；升级补发以现存记录为依据。

未发送公告/更新跨日保留。日期提醒错过前一天的运行后，只要目标时间仍未到达就可补发；只有日期、没有时刻的目标保留到当地目标日结束。发送成功后才记录回执；回执长期保留，网页提醒历史保留七天。
...
5. 工作流计划每天北京时间 02:00 完整采集，04:00、06:00、08:00 重试 pending，不重复爬取。UTC cron 分别是前一天 18:00、20:00、22:00 和当天 00:00。最后一轮通过 `--final-retry` 显式标记，按触发计划识别，不按实际启动时刻猜测。GitHub 定时任务可能延迟；运行仍尊重服务端 Retry-After 与额度冷却期。
...
`NOTIFY_EMAIL` 可填写多个纯邮箱地址，用英文逗号分隔，允许两侧空格；重复地址自动去重。所有地址仍放在同一个 Repository Secret 中。多收件人发送不在邮件头展示收件地址列表。逐邮箱发送，正常模式首次失败后最多重试 3 次；耗尽仍失败则进入不重试模式，后续每轮仅尝试一次，成功后恢复 3 次重试。当日成功的邮箱不会再发，全部成功后写入每日及任务回执。`state/email-delivery.json` 仅保存日期、当日成功 bitmap（sent）和不重试 bitmap（no_retry），不保存邮箱、邮箱摘要或 SMTP 响应。bit 位置对应 `NOTIFY_EMAIL` 去重后的配置顺序；修改收件人或顺序时必须同步迁移这些位，避免状态对应错误。SMTP 接受邮件表示本次投递成功，不保证最终入箱。
...
每个来源没有持久化 cursor 时会单独执行初始化：默认读取当前中国时区自然日和此前 20 个自然日，也就是三周；`INITIAL_LOOKBACK_DAYS=30` 可改为 30 天。页数上限按“自然日数量 × 2”计算，因此默认最多 42 页。初始化历史公告照常保存、筛选、提取和合并；通过筛选的历史候选合并进入当次的一封汇总，未来预约/开售等日期提醒另行保留，不因历史标记永久失去通知资格。
...
- `full`（默认）：抓取、候选筛选、AI 提取、Campaign 合并、到期通知、图片缓存与静态页全部执行。每天北京时间 02:00 执行，同时处理新材料和已有 pending。
- `retry`：只处理 pending，不爬取，并发布页面。pending 清空或指定 `--final-retry` 后按每日一封规则汇总发送；每日必发关闭且没有待通知事项时不发。
...
每日提醒页可切换今天及前六个自然日；未运行日期不会补造记录。state 保存每日快照，page 只发布摘要和原帖链接，过期详情不保留。更早快照、每日变更和旧倒计时队列自动清理；未发送公告／更新继续保留，直到成功投递。清理不会删除活动事实、来源材料或未来队列，也不会重写 Git 历史。

### 图片与邮件展示

详情页按原帖展示完整比例缩略图，点击可放大、前后切换、查看原尺寸长图；支持 Esc 关闭和键盘操作。邮件同时发送 HTML 卡片和纯文本备用版本，按新消息与时间提醒分组，使用内联样式，不依赖脚本或远程图片。

最后一轮可用 `python -m herald --phase retry --final-retry` 执行；普通 `retry` 在仍有 pending 时不会发邮件。发送成功并持久化每日回执后，当天后续运行不再发信。SMTP 发送与 Git 提交不是原子事务，发送后进程中断或 state 推送失败仍可能导致重发。
