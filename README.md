...
每次完整采集或 pending 重试结束，最多发送一封汇总：包含尚未发送的新公告/更新，以及历史日期队列中已到提醒日且目标尚未过期的内容。因此，即使官号今天没有再发微博，“明天开售”仍然可以按旧资料提醒。RETRY 没有新内容时不发邮件；当日已发日报不会阻止后续新增内容补发。
...
5. 以后工作流每天约在北京时间 08:15 自动运行；另在北京时间 02:15、14:15、20:15 重试 pending、汇总未发送且有效的提醒并更新网页，不重复爬取。GitHub 定时任务可能延迟。
...
- `retry`：只处理 pending，不爬取；处理结束后最多发送一封新增公告/更新与有效到期提醒的汇总，并发布页面。没有未发送内容时不发邮件。
